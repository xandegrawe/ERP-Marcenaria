...
class BankInvoiceService < ApplicationController
  ...
  def create(bank_invoice_params)
    return { error: 'Conta bancária não pode ser nula' } if bank_invoice_params[:bank_account_id].nil?
    ...
    ActiveRecord::Base.transaction do
      ...
      { bank_invoice:, message: 'Fatura criada com sucesso' }
  ...
  def formated_balance_view(balance)
    formatted_balance = format('%.2f', balance.to_f)
  ...
  def update(bank_invoice_params)
    ...
    filter_params[:amount] = formated_balance_db(filter_params[:amount].to_s)
    ...
    ActiveRecord::Base.transaction do
      bank_invoice.update!(category_id: filter_params[:category_id], person_id: filter_params[:person_id],
                           amount: filter_params[:amount], status: filter_params[:status], note: filter_params[:note])
      { bank_invoice:, message: 'Fatura atualizada com sucesso' }
    rescue ActiveRecord::RecordInvalid => e
      { error: e.message }
    end
  end
